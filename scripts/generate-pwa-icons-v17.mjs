/**
 * HYPER PWA Android icons v17 — enlarge the mark inside the black 446 viewport.
 *
 * Galaxy shows Chrome's adaptive crop: 512 → pad 79 → 670 → viewport 446 @ 33.
 * v16 left a readable but small stack inside that black field.
 * v17 keeps the same elements and the same black field, and fills the
 * 446 viewport as tightly as the squircle allows (no overflow past black).
 *
 * Does not overwrite v12–v16 or iOS apple-touch.
 * Run: node scripts/generate-pwa-icons-v17.mjs
 */
...
export async function composeSpec(srcBuf, size) {
  ...
  const stroke = Math.max(4, Math.round(spec.viewport * (6 / 446)))
  const frameInset = 2
  const innerPad = 2
  const contentSide = spec.viewport - (frameInset + stroke + innerPad) * 2
  const contentOrigin = spec.origin + frameInset + stroke + innerPad

  const lineH = Math.max(3, Math.round(contentSide * 0.012))
  const gapBooks = 2
  const gapLine = 2
  const gapKorean = 2
  const textBudget = Math.round(contentSide * 0.28)
  const kh = Math.max(14, Math.round(textBudget * 0.28))
  const hh = Math.max(28, Math.round(textBudget * 0.58))
  const bh = contentSide - hh - kh - lineH - gapBooks - gapLine - gapKorean
  const bw = contentSide
  const hw = contentSide
  const kw = Math.round(contentSide * 0.92)
  const lw = Math.round(contentSide * 0.72)
...
async function main() {
  ...
  if (outMeta.hasAlpha || outMeta.channels !== 3) throw new Error('v17 must stay opaque RGB')
  ...
  if (Buffer.compare(out512, readFileSync(join(root, TARGETS_512[1]))) !== 0) {
    throw new Error('v17 any/maskable 512 mismatch')
  }
  console.log(
    `wrote v17 any===maskable viewport=${spec.viewport} origin=${spec.origin} pad=${spec.padding} padded=${spec.padded}`,
  )
}
