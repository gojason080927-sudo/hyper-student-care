/**
 * HYPER ACADEMY PWA icon v10 — v9 typography tuning (C안).
 * Smaller H, HYPER held, larger ACADEMY, matched visual widths.
 * Same navy/silver, circular safe-zone, and same-master any/maskable as v9.
 * Does not overwrite v9 files.
 * Run: node scripts/generate-pwa-icons-v10.mjs
 */
...
const previewDir = join(root, 'public', '_preview-v10')
...
<style>
  ...
  .hyper {
    ...
    line-height: 0.86;
    letter-spacing: ${s.hyperTrack}em;
  ...
  .academy {
    ...
    line-height: 0.88;
    letter-spacing: ${s.academyTrack}em;
    margin: ${s.gap2}px 0 0;
    padding: 0;
    transform: scaleX(${s.academyScale});
    transform-origin: center;
  }
</style>
</head>
...
function analyze(data, width, height, channels) {
  ...
  const hB = found[0]
  const hyperB = found[1]
  const academyB = found[2]
  return {
    count,
    ...
    bands: found,
    widthRatio: hB && hyperB && academyB
      ? {
          h: 100,
          hyper: +((100 * hyperB.w) / hB.w).toFixed(1),
          academy: +((100 * academyB.w) / hB.w).toFixed(1),
        }
      : null,
  }
}
...
async function main() {
  ...
  for (let step = 0; step < 8; step++) {
    const [hB, hyperB, academyB] = stats.bands
    if (!hB || !hyperB || !academyB) break

    const target = hyperB.w
    const hRatio = hB.w / target
    const aRatio = academyB.w / target

    if (hRatio < 0.95) spec.hScale = clamp(+(spec.hScale * (0.97 + 0.03 / hRatio)).toFixed(3), 1.0, 1.28)
    else if (hRatio > 1.06) spec.hScale = clamp(+(spec.hScale * 0.98).toFixed(3), 1.0, 1.28)

    if (aRatio > 1.05) {
      spec.academyTrack = clamp(+(spec.academyTrack - 0.008).toFixed(3), -0.04, 0.03)
      spec.academyScale = clamp(+(spec.academyScale * 0.985).toFixed(3), 0.9, 1.05)
    } else if (aRatio < 0.95) {
      spec.academyTrack = clamp(+(spec.academyTrack + 0.006).toFixed(3), -0.04, 0.03)
      spec.academyScale = clamp(+(spec.academyScale * 1.012).toFixed(3), 0.9, 1.05)
    }

    if (!stats.insideCircle || stats.circleMargin < MIN_CIRCLE_MARGIN || stats.pctW > 56) {
      spec.academyScale = clamp(+(spec.academyScale * 0.98).toFixed(3), 0.9, 1.05)
      spec.hScale = clamp(+(spec.hScale * 0.985).toFixed(3), 1.0, 1.24)
      spec.hyperTrack = clamp(+(spec.hyperTrack - 0.004).toFixed(3), 0.0, 0.05)
    }
    ...
    const aligned =
      ...
      stats.pctH <= 68 &&
      stats.widthRatio &&
      stats.widthRatio.hyper >= 95 &&
      stats.widthRatio.hyper <= 105 &&
      stats.widthRatio.academy >= 95 &&
      stats.widthRatio.academy <= 105

    master = await renderIcon(page, spec)
    stats = await inspectBuffer(master, `try-${step + 1}`)
    if (aligned && Math.abs(stats.centerOffX) <= 2 && Math.abs(stats.centerOffY) <= 2) break
  }

  if (Math.abs(stats.centerOffX) > 0.5 || Math.abs(stats.centerOffY) > 0.5) {
  ...
  if (!stats.insideCircle || stats.circleMargin < 20) {
    throw new Error(
  ...
  await inspectBuffer(master, 'v10-master-1024')
  await inspectBuffer(any512, 'v10-any-512')
  await inspectBuffer(any512, 'v10-mask-512-same-as-any')
  await inspectBuffer(await sharp(any512).resize(48, 48).png().toBuffer(), 'v10-any-48')

  const v9 = await sharp(join(root, 'public', 'hyper-academy-icon-master-v9.png')).png().toBuffer()
  await inspectBuffer(v9, 'v9-master-compare')

  await writePng(join(root, 'public', 'hyper-academy-icon-master-v10.png'), master)
  const teacher = {
    'hyper-teacher-icon-v10-192.png': any192,
    'hyper-teacher-icon-v10-512.png': any512,
    'hyper-teacher-icon-maskable-v10-192.png': any192,
    'hyper-teacher-icon-maskable-v10-512.png': any512,
  }
  const parent = {
    'hyper-parent-icon-v10-192.png': any192,
    'hyper-parent-icon-v10-512.png': any512,
    'hyper-parent-icon-maskable-v10-192.png': any192,
    'hyper-parent-icon-maskable-v10-512.png': any512,
  }
  ...
  const v9s = await maskedPreview(v9, 48, 'samsung')
  const v10s = await maskedPreview(master, 48, 'samsung')
  const sMeta = await sharp(v9s).metadata()
  const gap = 8
  const samsungCompare = await sharp({
    create: {
      width: sMeta.width * 2 + gap + 16,
      height: sMeta.height + 16,
      channels: 4,
      background: { r: 228, g: 230, b: 234, alpha: 1 },
    },
  })
    .composite([
      { input: v9s, left: 8, top: 8 },
      { input: v10s, left: 8 + sMeta.width + gap, top: 8 },
    ])
    .png()
    .toBuffer()
  await writePng(join(previewDir, 'compare-samsung-v9-v10-48.png'), samsungCompare)

  const v9p = await sharp(v9).resize(48, 48).png().toBuffer()
  const v10p = await sharp(master).resize(48, 48).png().toBuffer()
  await writePng(
    join(previewDir, 'compare-v9-v10-48.png'),
    await sharp({
      create: { width: 48 * 2 + gap + 16, height: 48 + 16, channels: 4, background: { r: 228, g: 230, b: 234, alpha: 1 } },
    })
      .composite([
        { input: v9p, left: 8, top: 8 },
        { input: v10p, left: 8 + 48 + gap, top: 8 },
      ])
      .png()
      .toBuffer(),
  )

  console.log('done v10 spec=' + JSON.stringify(spec))
  console.log('widthRatio', stats.widthRatio)
}
