/**
 * class_schedule_grids RLS 정책 적용
 *
 * 환경 변수 (둘 중 하나):
 ...
 *
 * Usage:
 *   node scripts/apply-class-schedule-grids-rls.mjs
 */
...
if (!databaseUrl) {
  console.error(
    'DATABASE_URL 또는 SUPABASE_DB_PASSWORD 환경 변수가 필요합니다.\n' +
      'Supabase Dashboard → SQL Editor에서 supabase/class-schedule-grids-rls.sql 실행도 가능합니다.',
  )
  process.exit(1)
}
...
try {
  ...
  console.log('\n=== class_schedule_grids policies ===')
  for (const row of policies.rows) {
    console.log(`- ${row.policyname} (${row.cmd}) roles=${row.roles}`)
  }

  if (policies.rows.length === 0) {
    console.error('FAIL: no policies created')
    process.exit(1)
  }

  console.log('\nOK: class_schedule_grids RLS policies applied')
} catch (error) {
