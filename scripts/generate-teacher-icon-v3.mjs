/**
 * HYPER TEACHER PWA icons from the FULL official logo plate.
 * Crop/resize only — no glyph redraw.
 * Run: node scripts/generate-teacher-icon-v3.mjs
 */
...
const PREFIX = 'hyper-teacher-v3'

function isNearWhite(r, g, b) {
  return r > 230 && g > 230 && b > 230
}

/** Full logo plate: strip outer white margin, paint AA whites to navy. */
...
/** Maskable: full plate in ~82% safe zone (Android adaptive). */
...
async function write(name, buf) {
  ...
  console.log('wrote', name, buf.length, `${meta.width}x${meta.height}`)
}
...
async function main() {
  ...
  await write(`${PREFIX}-maskable-512.png`, await renderMaskable(plate, 512, 0.82))
