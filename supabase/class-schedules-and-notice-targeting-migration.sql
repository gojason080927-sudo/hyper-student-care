...
-- ---------------------------------------------------------------------------
-- 1. class_schedules (반별 공통 시간표)
-- ---------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS public.class_schedules (
  id             UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  grade          TEXT NOT NULL,
  class_name     TEXT NOT NULL,
  subject        TEXT NOT NULL CHECK (subject IN ('수학', '영어')),
...
CREATE INDEX IF NOT EXISTS idx_class_schedules_class
  ON public.class_schedules (grade, class_name, subject)
  WHERE is_active = true;

CREATE INDEX IF NOT EXISTS idx_class_schedules_active
  ON public.class_schedules (is_active);
...
-- 기존 공지: audience_type 기본값 'all' 유지

-- ---------------------------------------------------------------------------
-- 3. SQL helper — 수학A↔영수A, 수학B↔영수B 연동 (고1/중1/중3)
-- ---------------------------------------------------------------------------
...
-- 4. 시간표 노출: 학생 반/과목 기준
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public._schedule_visible_to_student(
  p_schedule public.class_schedules,
  p_grade text,
  p_class_name text
)
RETURNS boolean
...
DECLARE
  ...
  v_math_class text;
BEGIN
  IF NOT p_schedule.is_active THEN RETURN false; END IF;
  IF p_schedule.grade <> p_grade THEN RETURN false; END IF;

  -- 수학 전용반
  IF v_trimmed LIKE '% 수학A' OR v_trimmed LIKE '% 수학B' OR v_trimmed LIKE '% 수학' THEN
    IF p_schedule.subject = '수학' AND p_schedule.class_name = v_trimmed THEN RETURN true; END IF;
    RETURN false;
  END IF;

  -- 영어만
  IF v_trimmed LIKE '% 영어' AND v_trimmed NOT LIKE '% 영수%' THEN
    IF p_schedule.subject = '영어' AND p_schedule.class_name = v_trimmed THEN RETURN true; END IF;
    RETURN false;
  END IF;

  -- 영수반 (A/B/일반)
  IF v_trimmed LIKE '% 영수%' THEN
    v_linked := public._math_linked_class_names(p_grade, v_trimmed);
    v_math_class := v_linked[2]; -- may be math class when eng is first
    IF array_length(v_linked, 1) >= 2 THEN
      IF v_trimmed = v_linked[1] AND v_linked[2] IS NOT NULL THEN
        v_math_class := CASE WHEN v_linked[1] LIKE '% 수학%' THEN v_linked[1] ELSE v_linked[2] END;
      END IF;
    END IF;
    -- 수학 시간표: 연동된 수학반
    IF p_schedule.subject = '수학' THEN
      FOREACH v_math_class IN ARRAY v_linked LOOP
        IF v_math_class LIKE '% 수학%' AND p_schedule.class_name = v_math_class THEN RETURN true; END IF;
      END LOOP;
      RETURN false;
    END IF;
    -- 영어 시간표: 본인 반 또는 학년 공통 영어반
    IF p_schedule.subject = '영어' THEN
      IF p_schedule.class_name = v_trimmed THEN RETURN true; END IF;
      IF p_schedule.class_name = p_grade || ' 영어' THEN RETURN true; END IF;
    END IF;
    RETURN false;
  END IF;

  -- 기타: 본인 반 + 과목 일치 (초등/중2/고2 등)
  IF p_schedule.class_name = v_trimmed THEN
    IF v_trimmed LIKE '% 수학%' AND p_schedule.subject = '수학' THEN RETURN true; END IF;
    IF v_trimmed LIKE '% 영어%' AND p_schedule.subject = '영어' THEN RETURN true; END IF;
    IF v_trimmed LIKE '% 영수%' THEN RETURN true; END IF;
  END IF;
...
-- ---------------------------------------------------------------------------
-- 6. get_parent_care_bundle — 시간표·공지 학생별 필터
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    'class_schedules',
    coalesce(
      (SELECT jsonb_agg(to_jsonb(cs) ORDER BY cs.class_name, cs.day_of_week, cs.start_time)
       FROM public.class_schedules cs
       WHERE public._schedule_visible_to_student(cs, v_grade, v_class_name)),
      '[]'::jsonb
    ),
    'notices',
    coalesce(
