...
assert.deepEqual(
  ...
  [
    ['/teacher/hyper-teacher-icon-v19-192.png', '192x192', 'any'],
    ['/teacher/hyper-teacher-icon-v19-512.png', '512x512', 'any'],
    ['/teacher/hyper-teacher-icon-maskable-v19-192.png', '192x192', 'maskable'],
    ['/teacher/hyper-teacher-icon-maskable-v19-512.png', '512x512', 'maskable'],
  ],
)
...
assert.deepEqual(
  ...
  [
    ['/care/hyper-parent-icon-v19-192.png', 'any'],
    ['/care/hyper-parent-icon-v19-512.png', 'any'],
    ['/care/hyper-parent-icon-maskable-v19-192.png', 'maskable'],
    ['/care/hyper-parent-icon-maskable-v19-512.png', 'maskable'],
  ],
)
assert.deepEqual(
  hubManifest.icons.map((icon: { src: string; purpose: string }) => [icon.src, icon.purpose]),
  [
    ['/hub/hyper-hub-icon-v19-192.png', 'any'],
    ['/hub/hyper-hub-icon-v19-512.png', 'any'],
    ['/hub/hyper-hub-icon-maskable-v19-192.png', 'maskable'],
    ['/hub/hyper-hub-icon-maskable-v19-512.png', 'maskable'],
  ],
)

assert.match(types, /HUB_ACADEMY_LOGO_WEBP = '\/hub\/hyper-academy-logo-v11\.webp'/)
assert.match(indexHtml, /manifest\.href = '\/teacher\/manifest\.webmanifest\?v=19-installable'/)
assert.match(indexHtml, /hyper-teacher-icon-v19-192\.png/)
assert.match(indexHtml, /hyper-teacher-apple-touch-v12-180\.png/)
assert.doesNotMatch(indexHtml, /hyper-teacher-icon-v18-192\.png/)
assert.match(teacherRegistrar, /\/teacher\/manifest\.webmanifest\?v=19-installable/)
assert.match(parentRoute, /\/care\/manifest\.webmanifest\?v=19-installable/)
assert.match(hubSession, /\/hub\/manifest\.webmanifest\?v=19-installable/)
assert.match(middleware, /\/care\/manifest\.webmanifest\?v=19-installable/)
assert.match(middleware, /\/hub\/manifest\.webmanifest\?v=19-installable/)
assert.match(teacherLayout, /hyper-teacher-apple-touch-v12-180\.png\?v=12/)
assert.match(teacherLayout, /hyper-teacher-icon-v19-192\.png\?v=19/)
assert.match(parentRegistrar, /hyper-parent-apple-touch-v12-180\.png\?v=12/)
assert.match(parentRegistrar, /MANIFEST_VERSION = '19'/)
assert.match(hubRegistrar, /hyper-hub-apple-touch-v12-180\.png\?v=12/)
assert.match(hubRegistrar, /hyper-hub-icon-v19-192\.png\?v=19/)
assert.match(teacherPush, /hyper-teacher-icon-v19-192\.png/)
assert.match(careSw, /hyper-parent-icon-v19-192\.png/)
assert.match(hubSw, /hyper-hub-icon-v19-192\.png/)
