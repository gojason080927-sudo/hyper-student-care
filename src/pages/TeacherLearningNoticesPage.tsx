...
import { TeacherClassScheduleGridManager } from '../components/classSchedule/TeacherClassScheduleGridManager'
...
import { MakeupPlanPage } from './MakeupPlanPage'
...
import { filterNoticesForTeacherAdmin } from '../utils/noticeAudience'
import { btnPrimary, inputClass } from '../utils/labels'

type Tab = 'schedules' | 'notices' | 'makeup'
...
export function TeacherLearningNoticesPage() {
  ...
  const filtered = useMemo(() => {
    let list = filterNoticesForTeacherAdmin(contentPosts, includeExpired)
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="반별 수업 시간표, 학습 공지사항, 보강계획을 관리합니다."
        action={
          activeTab === 'notices' ? (
      ...
      <div className="flex flex-wrap gap-2 rounded-xl border border-slate-200 bg-white p-1 shadow-sm">
        <button
          ...
          className={`min-w-[30%] flex-1 rounded-lg px-3 py-2 text-sm font-semibold ${
            activeTab === 'schedules'
      ...
      {activeTab === 'schedules' ? (
        <TeacherClassScheduleGridManager />
      ) : activeTab === 'makeup' ? (
        <MakeupPlanPage embedded />
      ) : (
        <>
