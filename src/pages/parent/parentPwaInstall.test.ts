...
assert.equal(parentPwaManifestHref(''), '/care/manifest.webmanifest?v=10-installable')
assert.equal(parentPwaManifestHref('short'), '/care/manifest.webmanifest?v=10-installable')
...
assert.equal(careManifestHrefForPath('/care/'), '/care/manifest.webmanifest?v=10-installable')
...
assert.match(hubPatched, /\/hub\/manifest\.webmanifest\?v=1-installable&start=/)
assert.doesNotMatch(
  hubPatched,
...
try {
  ...
  const staticCare = await runMiddleware(
    new Request('https://hyper-student-care.vercel.app/care/manifest.webmanifest?v=10-installable', {
      headers: { 'user-agent': IOS_SAFARI },
    }),
  )
  ...
  assert.match(
    careRootHtml,
    /<link rel="manifest" id="app-manifest" href="\/care\/manifest.webmanifest\?v=10-installable" \/>/,
  )
  assert.doesNotMatch(
    careRootHtml,
  ...
  assert.match(hubHtml, /\/hub\/manifest\.webmanifest\?v=1-installable&start=/)
  assert.doesNotMatch(
    hubHtml,
    /<link rel="manifest" id="app-manifest" href="\/care\/manifest\.webmanifest/,
  )
