...
const HREF_A = `/hub/manifest.webmanifest?v=1-installable&start=${encodeURIComponent(START_A)}`
const HREF_B = `/hub/manifest.webmanifest?v=1-installable&start=${encodeURIComponent(START_B)}`
...
assert.match(
  patched,
  /<link rel="manifest" id="app-manifest" href="\/hub\/manifest.webmanifest\?v=1-installable" \/>/,
)
assert.doesNotMatch(
  patched,
...
assert.match(patchedA, /manifest\.href = '\/teacher\/manifest.webmanifest\?v=10-installable'/)
...
assert.equal(hubManifestHrefForPath('/hub/'), '/hub/manifest.webmanifest?v=1-installable')
...
assert.equal(hubPwaManifestHref(''), '/hub/manifest.webmanifest?v=1-installable')
...
try {
  ...
  const staticManifest = await runHubMiddleware(
    new Request('https://hyper-student-care.vercel.app/hub/manifest.webmanifest?v=1-installable', {
      headers: { 'user-agent': iphone },
    }),
  )
