...
const HREF_A = `/hub/manifest.webmanifest?v=19-installable&start=${encodeURIComponent(START_A)}`
const HREF_B = `/hub/manifest.webmanifest?v=19-installable&start=${encodeURIComponent(START_B)}`
...
assert.match(indexHtml, /\/hub\/manifest.webmanifest\?v=19-installable/)
assert.match(indexHtml, /encodeURIComponent\('\/hub\/' \+ hubKey\)/)

const patched = patchIndexHtmlForHub(indexHtml)
assert.match(
  patched,
  /<link rel="manifest" id="app-manifest" href="\/hub\/manifest.webmanifest\?v=19-installable" \/>/,
)
assert.doesNotMatch(
  patched,
...
assert.match(patched, /manifest\.href = '\/teacher\/manifest.webmanifest\?v=19-installable'/)
...
assert.match(patchedA, /manifest\.href = '\/teacher\/manifest.webmanifest\?v=19-installable'/)
...
assert.equal(hubManifestHrefForPath('/hub/'), '/hub/manifest.webmanifest?v=19-installable')
...
assert.equal(hubPwaManifestHref(''), '/hub/manifest.webmanifest?v=19-installable')
...
try {
  ...
  const staticManifest = await runHubMiddleware(
    new Request('https://hyper-student-care.vercel.app/hub/manifest.webmanifest?v=19-installable', {
      headers: { 'user-agent': iphone },
    }),
  )
  ...
  assert.match(careHtml, /\/care\/manifest\.webmanifest\?v=19-installable&start=/)
