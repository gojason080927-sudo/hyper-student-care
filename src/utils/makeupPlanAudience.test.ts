...
const makeupEditStart = dataHook.indexOf('const saveMakeupPlanRecord')
const makeupCreateStart = dataHook.indexOf('const saveMakeupPlanRecords')
const makeupDeleteStart = dataHook.indexOf('const deleteMakeupPlanRecord')
const noticeStart = dataHook.indexOf('const saveContentPost')
assert.ok(makeupEditStart >= 0 && makeupCreateStart > makeupEditStart)
assert.ok(makeupDeleteStart > makeupCreateStart && noticeStart > makeupDeleteStart)
assert.doesNotMatch(dataHook.slice(makeupEditStart, makeupCreateStart), /notifyHubPush/)
assert.match(dataHook.slice(makeupCreateStart, makeupDeleteStart), /await upsertMakeupPlans\(records\)/)
assert.match(dataHook.slice(makeupCreateStart, makeupDeleteStart), /event: 'makeup_plan_saved'/)
assert.match(dataHook.slice(makeupCreateStart, makeupDeleteStart), /studentIds: uniqueIds/)
assert.doesNotMatch(dataHook.slice(makeupDeleteStart, noticeStart), /notifyHubPush/)
