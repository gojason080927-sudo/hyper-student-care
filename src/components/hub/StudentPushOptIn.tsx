...
import { ensureStudentPushSubscription, getStudentPushUiState, subscribeStudentPush } from '../../lib/studentPushClient'

type Props = {
  accessKey: string
}

export function StudentPushOptIn({ accessKey }: Props) {
  ...
  useEffect(() => {
    let cancelled = false
    void getStudentPushUiState(accessKey).then((state) => {
      if (cancelled) return
      ...
      if (state.permission === 'granted') {
        void ensureStudentPushSubscription(accessKey)
          .then(() => {
            if (cancelled) return
  ...
  const handleSubscribe = async () => {
    ...
    setHint('')
    try {
      await subscribeStudentPush(accessKey)
      setStatusLabel('알림 켜짐')
      setCanRequest(false)
    } catch (error) {
      setHint(error instanceof Error ? error.message : '알림 등록에 실패했습니다.')
  ...
  return (
    <section className="mt-4 rounded-2xl bg-white px-3 py-2.5 shadow-sm" data-student-push-opt-in="true">
